...
function onSubmit() {
  if (!validate()) {
    showToast('检票信息有误，请检查', 'error')
    return
  }
  showToast('检票成功，正在进入影院…', 'success')
  // 真实项目在这里调用登录接口，例如：
...
<template>
  <section class="ticket" role="form" aria-label="登录">
    ...
    <p class="ticket__hint">放映还未开始，请先检票入场。</p>
    ...
    <div class="divider"><span>其他检票方式</span></div>

    <div class="socials">
      <button class="social" type="button" aria-label="微信登录" @click="onSocial('微信')">
      ...
      <button class="social" type="button" aria-label="Apple 登录" @click="onSocial('Apple')">
        <svg viewBox="0 0 24 24" width="22" height="22">
          <path
            fill="currentColor"
      ...
      <button class="social" type="button" aria-label="Google 登录" @click="onSocial('Google')">
        <svg viewBox="0 0 24 24" width="22" height="22">
          <path
            fill="#EA4335"
    ...
    <p class="signup">还没有账号？<a href="#" @click.prevent="onSignup">立即购票注册</a></p>
